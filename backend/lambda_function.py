...
def call_gemini_api(model, system_instruction, prompt, is_json=False, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Solo reintentamos si es 503 (sobrecarga) o 429 (rate limit)
            if e.code in (503, 429) and attempt < max_retries - 1:
                time.sleep(2 ** attempt)  # espera 1s, luego 2s, luego 4s
                continue
...
def lambda_handler(event, context):
    try:
        ...
        # FASE 1 — Clasificador (Gemini Flash)
        system_filtro = (
